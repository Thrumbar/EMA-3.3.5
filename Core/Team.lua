...
local function AddMember( importName, class )
	...
	if name ~= nil and name:trim() ~= "" and name:len() > 1 then
		...
		if EMA.db.newTeamList[character] == nil then
			...
			local _, classFileName = UnitClass(character)
...
function EMA.DoTeamPartyInvite()
	EMA:Print("test", EMA.inviteList[EMA.currentInviteCount] ) 
...
function EMA:InviteTeamToParty( info, tag )
	...
	if EMAApi.DoesGroupExist(tag) == true then
		--EMA:Print("test", EMA.characterName, tag )
		if EMAApi.IsCharacterInGroup( EMA.characterName, tag ) == false then
			--EMA:Print("IDONOTHAVETAG", tag)
			for index, name in TeamListOrderedOnline() do
				local characterName, realm = EMAUtilities:splitNameAndRelam( name )
			...
			for index, name in TeamListOrderedOnline() do
				local characterName, realm = EMAUtilities:splitNameAndRelam( name )
				EMA:Print("test11", characterName, realm )
				if EMAApi.IsCharacterInGroup( name, tag ) == true then
					EMA:Print("HasTag", characterName, tag )
					-- As long as they are not the player doing the inviting.
					if name ~= EMA.characterName then
...
function EMA:PARTY_INVITE_REQUEST( event, inviter, ... )
	...
	if GetNumPartyMembers() == 0 then	
		...
		if EMA.db.inviteAcceptTeam == true then
			...
			local characterName = EMAUtilities:AddRealmToNameIfMissing( inviter )
			EMA:Print("test", IsCharacterInTeam( characterName ) )
...
function EMA:UpdateMacros()
	...
	local characterName = EMA.db.master
	...
	local EMAFocusOneName = EMAApi.GetCharacterNameAtOrderPosition(1)
		EMAFocusOne:SetAttribute( "macrotext", "/focus " .. EMAFocusOneName  )
	local EMAFocusTwoName = EMAApi.GetCharacterNameAtOrderPosition(2)
		EMAFocusTwo:SetAttribute( "macrotext", "/focus " .. EMAFocusTwoName  )
	local EMAFocusThreeName = EMAApi.GetCharacterNameAtOrderPosition(3) 
		EMAFocusThree:SetAttribute( "macrotext", "/focus " .. EMAFocusThreeName  )
	local EMAFocusFourName = EMAApi.GetCharacterNameAtOrderPosition(4) 
		EMAFocusFour:SetAttribute( "macrotext", "/focus " .. EMAFocusFourName  )
	local EMAFocusFiveName = EMAApi.GetCharacterNameAtOrderPosition(5) 
		EMAFocusFive:SetAttribute( "macrotext", "/focus " .. EMAFocusFiveName  )
	local EMAFocusSixName = EMAApi.GetCharacterNameAtOrderPosition(6)
		EMAFocusSix:SetAttribute( "macrotext", "/focus " .. EMAFocusSixName  )
	local EMAFocusSevenName = EMAApi.GetCharacterNameAtOrderPosition(7)
		EMAFocusSeven:SetAttribute( "macrotext", "/focus " .. EMAFocusSevenName  )
	local EMAFocusEightName =EMAApi.GetCharacterNameAtOrderPosition(8) 
		EMAFocusEight:SetAttribute( "macrotext", "/focus " .. EMAFocusEightName  )
	local EMAFocusNineName = EMAApi.GetCharacterNameAtOrderPosition(9)
		EMAFocusNine:SetAttribute( "macrotext", "/focus " .. EMAFocusNineName  )
	local EMAFocusTenName = EMAApi.GetCharacterNameAtOrderPosition(10)
...
function EMA:SettingsTeamListScrollRefresh()
	...
	for iterateDisplayRows = 1, EMA.settingsControl.teamList.rowsToDisplay do
		...
		EMA.settingsControl.teamList.rows[iterateDisplayRows].highlight:SetTexture( 0.0, 0.0, 0.0, 0.0 )
		-- Get data.
		...
		--local class = EMA.db.characterClass[characterName]
			...
			if dataRowNumber == EMA.settingsControl.teamListHighlightRow then
				EMA.settingsControl.teamList.rows[iterateDisplayRows].highlight:SetTexture( 1.0, 1.0, 0.0, 0.5 )
			end
		end
	end
end
...
function EMA:SettingsGroupListScrollRefresh()
	...
	for iterateDisplayRows = 1, EMA.settingsControl.groupList.rowsToDisplay do	
		...
		EMA.settingsControl.groupList.rows[iterateDisplayRows].highlight:SetTexture( 0.0, 0.0, 0.0, 0.0 )
		...
		--local group = EMAApi.GetGroupAtPosition( dataRowNumber )
			...
			if dataRowNumber == EMA.settingsControl.groupListHighlightRow then
				EMA.settingsControl.groupList.rows[iterateDisplayRows].highlight:SetTexture( 1.0, 1.0, 0.0, 0.5 )	
			end
		end	
	end
