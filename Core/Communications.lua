...
local function DefaultCommand( message )
	...
	if raidMembers > 10 and raidMembers < 41 then
			channel = "RAID"
	elseif partyMembers > 0 and partyMembers < 6 then
		channel = "PARTY"
	end		
	...
	for characterName, characterOrder in EMAPrivate.Team.TeamList() do		
		local name, realm = EMAUtilities:splitNameAndRelam( characterName )
		if UnitInParty( name ) == nil then				
			--EMA:Print( "Toon not in party:", name)
			if IsCharacterOnline( characterName ) == true then
				...
				--EMA:Print("testWis", EMA.COMMAND_PREFIX, EMA.COMMUNICATION_WHISPER, name , EMA.COMMUNICATION_PRIORITY_ALERT)	
			end	
		end
	end	
end
