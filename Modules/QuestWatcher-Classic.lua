...
function EMA:EMAQuestWatcherQuestLogUpdate( useCache )
	...
	local questIndex = GetQuestIndexForWatch( iterateWatchedQuests )
		...
		--EMA:Print("EMAQuestData", questLogTitleText, level, questTag,  suggestedGroup, isHeader, isCollapsed,  isComplete, isDaily, questID )										
		local numObjectives = GetNumQuestLeaderBoards( questIndex )							 
			for iterateObjectives = 1, numObjectives do
				...
				if objectiveFullText ~= nil then																												
					--EMA:Print("test2", questID, questLogTitleText, iterateObjectives, objectiveText, amountCompleted, objectiveFinished, objectiveFinished )
					if (EMA:QuestCacheUpdate( questID, iterateObjectives, amountCompleted, objectiveFinished ) == true) or (useCache == false) then
						EMA:EMASendCommandToTeam( EMA.COMMAND_QUEST_WATCH_OBJECTIVE_UPDATE, questID, questLogTitleText, iterateObjectives, objectiveText, amountCompleted, objectiveFinished, objectiveFinished )																		   
					end
				end
			end
		end
